/* solution-par.c (Matheus Lima Ferreira 1 mar. 2021)
 * Compilation: gcc solution-par.c -o solution-par -fopenmp
...
int maximum_sum_subsequence(int *arr, int n, int k)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        dp[i][1] = arr[i];
        for (j = 2; j <= k; j++)
            // matriz para realizar o calculo do somatorio
            dp[i][j] = -1;
    }

    // paralelizar este laço para obter mais performance
    for (l = 1; l <= k - 1; l++) {
